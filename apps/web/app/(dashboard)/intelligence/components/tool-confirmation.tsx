...
export function ToolConfirmation({ toolName, input, onApprove, onReject, isExecuting }: ToolConfirmationProps) {
  ...
  useEffect(() => {
    if (toolName === "publishCampaign" && input.campaignId) {
      fetch(`/api/meta/campaigns/${input.campaignId}`)
        .then((r) => r.json())
        .then((data) => {
          const name = data?.[0]?.campaigns?.name || data?.campaign?.name || data?.name;
          if (name) setCampaignName(name);
        })
        .catch(() => {});
    }
  }, [toolName, input.campaignId]);
  ...
  return (
    <div className="my-2 max-w-md rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/30">
      ...
      <div className="mb-3 space-y-1">
        {toolName === "publishCampaign" && campaignName && (
          <div className="flex gap-2 text-xs">
            <span className="shrink-0 font-medium text-muted-foreground">Kampanya:</span>
